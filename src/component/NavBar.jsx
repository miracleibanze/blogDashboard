    ...
    const content = <>
        <div className=" md:hidden flex items-center justify-center h-screen fixed top-0 w-full left-0 right-0 bg-slate-800 transition-all z-[999]">
            <ul className="text-center text-xl p-20 min-w-[300px] w-[90%]">
                    {pages.map((page) => (
                       <li key={page.id} className="relative flex items-center my-2 cursor-pointer py-2 border-b border-slate-300 hover:bg-slate-300 hover:text-n-8" 
                       onClick={() => {
    ...
    <nav className="max-w-full overflow-x-hidden" id="page">
        <div className="relative w-full h-[7rem] max-md:h-[5rem] flex bg-n-1 px-16 py-4 max-md:px-4 max-md:py-2 items-center justify-between text-n-8/90">
          <div className="flex items-center relative">
            ...
            />
            <h4 className="text-start pl-2 ml-2 max-md:pl-1 max-md:ml-1 max-md:h-[3rem] max-md:leading-none border-l-2 border-color-3 text-lg max-md:text-[1rem] leading-5 font-bold h-[5rem] flex flex-col justify-center items-start">Ecole<br/>Secondaire&nbsp;De<br/><font color='green'>Mukingi</font></h4>
          </div>
        ...
        <div className="h-[10vh] w-full flex justify-between z-50 lg:py-5 px-20 max-md:px-10 py-4">
            <div className="lg:flex md:flex flex-1 items-center font-normal">
                ...
                <ul className="flex gap-8 mr-16 text-[18px]">
                    {pages.map((page) => (
                       <li key={page.id} className="hover:text-fuchsia-600 transition hover:border-b-2 hover:border-fuchsia-600 cursor-pointer flex items-center" onClick={() => {
                        setSlideIndex(page.favSlide)
                        setPageIndex(page.id)
                       }}>
                        <img 
