...
const Carousel = ({ pageIndex, pages, slides, prevSlide, nextSlide, goToIndex, slideIndex }) => {
    ...
    <div className="w-full h-[780px] max-md:h-[425px]">
      ...
      >
        <div className='w-full h-full absolute z-2 group'>
          <div className='absolute flex items-center pl-8 h-full md:w-32 max-md:max-w-16 bg-gradient-to-l from-n-8/10 to-n-8/90 cursor-pointer' onClick={prevSlide}>
            <img 
          ...
          <div className='absolute flex items-center right-0 pr-8 h-full w-32 justify-end  bg-gradient-to-r from-n-8/10 to-n-8/90 cursor-pointer' onClick={nextSlide}>
            <img 
      ...
      <div className='relative w-full h-[50px] max-md:h-[25px] flex items-center justify-center gap-4 max-md:gap-3 bg-slate-900 -top-4'>
        {slides.map((item, itemIndex) => (
         <div key={itemIndex}>
           <img src={dot} className={`w-4 h-4 max-md:w-3 max-md:h-3 cursor-pointer rounded-full ${ slideIndex != itemIndex ? '' : 'bg-white'}`} 
