...
import { grid, headMaster, schoolBoys, starFull, studentTalk} from './../assets'
  ...
  const news = [
    {
      ...
      poster: headMaster,
      content: "In this parental meeting on 14 November, 2024, School headMaster anounced that who ever student who will not come below a half of conduct marks, he/ she won't have another chance of staying in this school. Along with other decisions acording to finances, living health of students and other updates.",
    },
    {
    ...
    <>
      <div className='flex justify-center w-full'>
          {newTypeTitle.map((item, itemIndex) => (
            <div key={itemIndex}
            className={`relative h3 font-bold flex flex-col items-center justify-center cursor-pointer w-1/2 p-8 ${ newsType != itemIndex ? '' : 'text-n-8/90 bg-n-1/50'}`} 
            onClick={() => {
            ...
            >
              ...
              {item.span && <Tagline className={`body-2 text-n-1/80 ${!newsType != itemIndex ? 'hidden' : ''}`}>{item.span}</Tagline>}
            </div>
          ))}
          {newsType}
        </div>
      <NewsPictureHighlight className={`${newsType != 0 ? 'block' : 'hidden'}`} />
      <Section className={`${newsType != 1 ? 'block' : 'hidden'}`}>
        <div className="container relative">
          <Heading title='latest News' span='& Events' text='Here you can read and follow what happened in school that you might need to know' />
          <div className="relative grid gap-6 md:grid-cols-2 md:gap-4 md:pb-[7rem]">
          {news.map((item, itemIndex) => (
                <div key={itemIndex} className='md:flex even:md:translate-y-[7rem] p-[0.065rem] rounded-[2.5rem] bg-conic-gradient'>
                  <div className="relative p-8 bg-color-5 rounded-[2.5rem] overflow-hidden xl:p-15 w-full h-full">
                    ...
                    <div className="relative z-1 ">
                      ...
                      <h4 className="h4 mb-4 flex items-center justify-between">{item.name}
                      </h4>
                      <p className="body-1 text-n-3 text-justify">{item.content}</p>
